...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "phantomx " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. phantomx is probably already running."), addrBind.ToString());
        else
