...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizephantomxAmount"))
        settings.setValue("nAnonymizephantomxAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizephantomxAmount = settings.value("nAnonymizephantomxAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizephantomxAmount"))
        SoftSetArg("-anonymizephantomxamount", settings.value("nAnonymizephantomxAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizephantomxAmount:
            nAnonymizephantomxAmount = value.toInt();
            settings.setValue("nAnonymizephantomxAmount", nAnonymizephantomxAmount);
            emit anonymizephantomxAmountChanged(nAnonymizephantomxAmount);
            break;
        default:
            break;
        }
