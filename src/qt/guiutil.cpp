...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("phantomx"))
        return false;
...
{
    // Convert phantomx:// to phantomx:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("phantomx://"))
    {
        uri.replace(0, 12, "phantomx:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "phantomx.lnk";
}
...
{
    return GetAutostartDir() / "phantomx.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("phantomx-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  phantomx-qt [" + tr("command-line options") + "]                     " + "\n";
