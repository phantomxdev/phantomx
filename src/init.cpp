...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: phantomx.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: phantomxd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizephantomxamount=<n> " + _("Keep N phantomx anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. phantomx is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. phantomx is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of phantomx") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart phantomx to complete") << "\n";
    ...
    nAnonymizephantomxAmount = GetArg("-anonymizephantomxamount", 0);
    if(nAnonymizephantomxAmount > 999999) nAnonymizephantomxAmount = 999999;
    if(nAnonymizephantomxAmount < 2) nAnonymizephantomxAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizephantomxAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize phantomx Amount %d\n", nAnonymizephantomxAmount);
